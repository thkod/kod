...
USER::USER()
{
	...
	for(int i=0;i<USERFUNCNUMMAX;i++){
		if(i==0)
            ...
            UserFunc[i] = &USER::Formula_to_surface;	// 曲面生成
        else if(i==5)
            UserFunc[i] = &USER::Texture_arrangement;	// 未使用のUserFunc[i]にはMasterUserFunc()を登録しておく
        else if(i==6)
...
//###################################################↓編集中############################################################
// Function: Formula_to_surface

int USER::Formula_to_surface(BODYList *BodyList, OBJECTList *ObjList, int PickCount)
{

     EqtoSurface(BodyList,ObjList,PickCount,UserStat.Prop);

     return KOD_TRUE;
}

int USER::Texture_arrangement(BODYList *BodyList, OBJECTList *ObjList, int PickCount)
{
	/*
	NURBSS *S1;
    NURBSC *S2;
	 // 一つ目に配置先の平面，二つ目にテクスチャを選択
    for(int i=0;i<PickCount;i++){
        OBJECT *obj = (OBJECT *)ObjList->getData(i);			// 一番最初にセレクションされたエンティティの情報を得る
        BODY *body = (BODY *)BodyList->getData(obj->Body);		// 一番最初にセレクションされたBODYの実体を得る
        if(i==0)
            S1 = body->TrmS[obj->Num].pts;
        else if(i==1)
            S2 = body->TrmS[obj->Num].pts;
    }
	*/
     TxArr(BodyList,ObjList,PickCount,UserStat.Prop);
...
void USER::SetMenuLabelName()
{
	...
	strcpy(MenuLabel5,"Formula_to_surface");
    strcpy(MenuLabel6,"Texture_arrangement");
...
void USER::RegistUserCommand(CommandMap *CmdMap)
{
	...
	// 第5引数はNULLを指定
    //SetCmdList(CmdMap,"eqtosurface","",CMD_CROSS,NULL);
}
